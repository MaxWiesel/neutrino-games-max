...
//	FBFillRect( 0, 0, 720, 576, BLACK );// orginal
	FBFillRect( 0, 0, 576, 526, BLACK ); // by bazi98
...
//	FBDrawFx2Logo( LOGO_X, LOGO_Y );//orginal
	FBDrawString( 490, 30, 64, "fx", WHITE, 0 );// by bazi98
	FBDrawString( 510, 40, 64, "2", RED, 0 );// by bazi98
...
static	int	GameOver( int mask )
{
	...
	if ( !k )
		return 0;
	if ( k == 2 ) {	// patt
//		FBDrawString( 190, 410, 64, "Good Game !", WHITE, 0 ); // orginal
		FBDrawString( 140, 430, 64, "Good Game !", WHITE, 0 ); // by bazi98
	} else if ( mask == 1 ) {
//		FBDrawString( 190, 410, 64, "You won !", WHITE, 0 ); // orginal
		FBDrawString( 140, 430, 64, "You won !", WHITE, 0 ); // by bazi98
	} else {
//		FBDrawString( 190, 410, 64, "Iam the Winner !", WHITE, 0 ); // orginal
		FBDrawString( 140, 430, 64, "Iam the Winner !", WHITE, 0 ); // by bazi98
	}
	doexit=1;
	return 1;
}
